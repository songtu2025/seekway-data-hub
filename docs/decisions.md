...
## 2026-09-18 查询日期范围报告接入决策

- `date_range_reports_page` 以公开文档 `id=27` 为契约来源，使用 `POST /finance/asset/dateRangeReports/page`，默认关闭自动执行并由后续受控发布创建账号关闭态策略。
- 历史回补下界采用用户确认的 `2021-08-01`；`queryDateType=0` 明确使用市场时间，单日窗口降低财务明细接口的单批风险。
- 官方 `data.rows.id` 作为幂等主键，`marketDate` 作为业务日期；不设置未经真实数据证明的分页上限，实时 `data.total` 是分页终止依据。
- 订单号、地区/邮编和金额字段只进入既有受限 raw 存储，本轮不新增解析表、财务报表页面或业务字段。

## 2026-09-18 上游限流恢复决策

- HTTP 429 与 509 均按上游限流处理；正常请求仍遵守官方基础频率，不以提高频率或并发规避网关限制。
- `date_range_reports_page` 命中限流后对同一方法与路径设置 65 秒共享冷却，并最多重试当前页 3 次；成功页不回滚、不重复从第一页开始。
- 限流与重试属于当前运行安全策略，不属于任务业务事实快照；旧任务执行时采用已发布配置中的最新 `rate_limit` 与 `retry`，路径和其余业务配置仍保持冻结。
- 敏感接口的失败证据只允许保存 `UPSTREAM_RATE_LIMIT`、HTTP 状态和短数字业务码，不保存请求参数、响应正文或上游原始消息。

## 2026-09-18 MySQL 聚合时间兼容决策

- Web 查询层不能假定 MySQL 聚合日期结果一定经过 SQLAlchemy `DateTime` 处理器；公共 UTC 序列化入口同时接受 `datetime` 和数据库 ISO 时间字符串，再统一输出带 `Z` 的 UTC 字符串。
- 该兼容只修正返回类型差异，不修改查询语义、数据库列、时区口径或 API 响应契约。
